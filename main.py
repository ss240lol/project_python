...
@njit()
def new_frame(posx, posy, rot, frame, sky, floor, hres, halfvres, mod, maph, size, wall, mapc, exitx, exity, nenemies, rotv):
...
def get_sprites(hres):
   sheet = pg.image.load(resource_path('zskeleton.png')).convert_alpha()
   sprites = [[], []]
   swordsheet = pg.image.load(resource_path('sword1.png')).convert_alpha()
   sword = []
   for i in range(3):
...
def main():
   pg.init()
   pg.mixer.init()
   pg.mixer.music.load(resource_path('background.mp3'))
   pg.mixer.music.set_volume(0.1)
   pg.mixer.music.play(-1)
   #sounds_step = pg.mixer.Sound(resource_path('step.wav'))
   #sounds_step.set_volume(0.3)
   #sounds_sword = pg.mixer.Sound(resource_path('sword.wav'))
   #sounds_damage = pg.mixer.Sound(resource_path('damage.wav'))
   #sounds_death = pg.mixer.Sound(resource_path('death.wav'))
   ...
   sky = pg.image.load(resource_path('skybox2.jpg'))
   sky = pg.surfarray.array3d(pg.transform.smoothscale(sky, (720, halfvres*4)))/255
   floor = pg.surfarray.array3d(pg.image.load(resource_path('floor.jpg')))/255
   wall = pg.surfarray.array3d(pg.image.load(resource_path('wall.jpg')))/255
   ...
   while running:
       ...
       pg.display.set_caption("Health: "+str(round(player_health*10))+"/100"+" Enemies: " + str(nenemies) + " FPS: " + str(fps))
